...
$payload = jwt_verify($_COOKIE['token'] ?? '');
if (!$payload) {
...
$isFirst = isset($_GET['first'], $_GET['login'], $_GET['password']);
...
<?php if ($isFirst): ?>
  <div class="success">
    <p><strong>Профиль успешно создан.</strong></p>
    <p>Логин: <strong><?= htmlspecialchars($_GET['login']) ?></strong></p>
    <p>Пароль: <strong><?= htmlspecialchars($_GET['password']) ?></strong></p>
    <p>Сохраните эти данные. Пароль больше не отображается.</p>
  </div>
<?php endif; ?>
